...
def main():
    ...
    parser.add_argument("--backend", default=["onnx"], help="backend to use. One of [onnx,tensorrt] or both", nargs='*', choices=["onnx", "tensorrt", "pytorch"])
    ...
    if "tensorrt" in args.backend:
        ...
        conf = Configuration(model_name=args.name, model_type=ModelType.TensorRT, batch_size=0, nb_output=output_pytorch.shape[1], nb_instance=1, include_token_type=include_token_ids, workind_directory=args.output)
    ...
    if "onnx" in args.backend:
        ...
        conf = Configuration(model_name=args.name, model_type=ModelType.ONNX, batch_size=0, nb_output=output_pytorch.shape[1], nb_instance=1, include_token_type=include_token_ids, workind_directory=args.output)
