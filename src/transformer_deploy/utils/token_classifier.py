...
from transformers import AutoConfig, AutoTokenizer, PreTrainedModel, PreTrainedTokenizer, TokenClassificationPipeline
...
class BertForTokenClassification(PreTrainedModel):
    ...
    def forward(
        ...
        if inference_response.has_error():
            ...
            return TokenClassifierOutput(logits=tensor)
...
class TritonPythonModel:
    ...
    def execute(self, requests) -> "List[List[pb_utils.Tensor]]":
        ...
        for request in requests:
            ...
            outputs = self.pipeline(query)
            # convert np.float32 to make json dump possible
